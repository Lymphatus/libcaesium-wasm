import CaesiumWASM from '../target/wasm32-unknown-emscripten/release/libcaesium-wasm.js';
...
export interface ILibcaesium {
    ...
    cwrap: (name: string, returnType: any, argTypes: string[]) => (...args: any[]) => number;
    ready: Promise<void>;
    asm: {
...
export function compress(ui8a: Uint8Array, options: CompressionOptions): CompressionResult {
    ...
    let ptr = libcaesium._malloc(ui8a.length);
    ...
    let status = libcaesium.getValue(dataPtr, 'i32')
    let errorCode = libcaesium.getValue(dataPtr + 4, 'i32')
    let vecPtr = libcaesium.getValue(dataPtr + 8, 'i32');
    let len = libcaesium.getValue(dataPtr + 12, 'i32');
    ...
    let drop_vector_struct = libcaesium.cwrap('drop_vector_struct', null, ['number']);
